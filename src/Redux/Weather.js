...
export const fetchData = (location) => async (dispatch) => {
  try {
    const coodUrl = `https://api.openweathermap.org/data/2.5/weather?q=${location}&i&appid=3e8619f5f4e5ee08671048f8899fe471`;
    const responseCord = await axios.get(coodUrl);
    const { lon, lat } = responseCord.data.coord;
    const url = `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=hourly&appid=3e8619f5f4e5ee08671048f8899fe471`;
